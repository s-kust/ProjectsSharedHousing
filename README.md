These are my Python/Django projects gathered under one roof. Before deployment, prepare the required data for configuration files `db_config.ini` and `/app/config.json`.  Most importantly, you'll need SMTP server data and private_key to access the Google Spreadsheet via its API.
...
# Portfolio Positions Reports

It was originally a Python script that I later packaged as a Django application. The main working code is in the file `portfolio_report_images_not_delete.py`. The script works with stocks, ETFs, and currency pairs. It loads a list of tickers from Google Spreadsheet, then for each position generates a report with charts and sends it to the email. You can easily change the set of items in the watchlist using your smartphone, as long as it works with Google Spreadsheets. 

![Watchlist spreadsheet example](/misc/1.PNG)

How it works: 
1. Load a list of tickers from Google Spreadsheet.
2. For each ticker, import the daily prices data from Alpha Vantage.
3. For each line, determine if it is a single ticker idea or two tickers relative performance spread.
4. Generate appropriate reports with charts and text notes and send them to the specified email address. 

You can create Celery tasks to run the script regularly at suitable intervals. For example, on business days in the morning and again 15 minutes before the end of the trading time.
...
Unfortunately, the initial setup of the script is quite complicated and requires having skills. Currently, I do not see any way to simplify it. Instead of a free Python script for your self-use, I could create a commercial online service. Such a service would be convenient but not free. Probably there is not enough demand for such a paid service.

During the initial setup of the script, difficulties may arise at the following stages:
1. Connecting to the SMTP server. 
1. Planning the regular launch of the task on the server.
1. Interactions with the Google spreadsheet. 

The listed tasks are best solved sequentially, and not all together.

1. Arrange access to the SMTP server for sending the reports to your mailbox. 
2. Obtain your unique API key from Alpha Vantage. It is free of charge. 
3. Fill the settings in the `config.ini` file using the provided example. 
4. Prepare the Google spreadsheet with the list of tickers to be traced. To enable the script to work with that spreadsheet, follow [these instructions](https://www.twilio.com/blog/2017/02/an-easy-way-to-read-and-write-to-a-google-spreadsheet-in-python.html). If the page is not available, use the [archived PDF version](misc/Google_Spreadsheets_Python.pdf).
...
5. Load the Python/Django code to the server and ensure that the script works OK when launched manually.
6. If everything works fine, schedule a regular run of the script uising Celery.

# Django Advanced Crispy Forms Usage Example
